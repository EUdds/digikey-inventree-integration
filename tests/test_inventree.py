...
def test_api(test_data):
    yield InvenTreeAPI(test_data['test_config']['INVENTREE_API']['URL'], username=test_data['test_config']['INVENTREE_API']['USER'], password=test_data['test_config']['INVENTREE_API']['PASSWORD'])
...
def test_get_digikey_supplier_existing_company(test_supplier_data, monkeypatch):
...
def test_create_inventree_part(monkeypatch, test_data):
...
def test_create_inventree_part_part_exists(test_data, monkeypatch):
    monkeypatch.setattr(test_module, 'find_category', lambda *args, **kwargs: inventree.part.PartCategory(test_api, data={"pk": 1}))
    monkeypatch.setattr(inventree.part.Part, 'uploadImage', lambda *args, **kwargs: None)
    monkeypatch.setattr(inventree.part.Part, 'create', mock_create_part)
...
def test_find_manufacturer(test_data, monkeypatch):
    monkeypatch.setattr(inventree.company.Company, 'list', lambda *args, **kwargs: [])
    monkeypatch.setattr(inventree.company.Company, 'create', mock_create_company)
...
def test_find_manufacturer_manufactuer_exists(test_data, monkeypatch):
...
def test_add_digikey_part(test_data, monkeypatch):
