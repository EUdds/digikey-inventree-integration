...
class TestSupplierPart:
    def test_has_required_fields(self, class_under_test, test_data):
        resp = test_data["test_api_responses"][f"{class_under_test.SUPPLIER_NAME}_resp"]
        part = class_under_test(resp, injest_api_automatically=False)
        for field in REQUIRED_FIELDS:
            assert hasattr(
                part, field
            ), f"Instance of class {class_under_test} is missing required field {field}"

    def test_filled_required_fields(self, class_under_test, test_data):
        test_resp = test_data["test_api_responses"][
            f"{class_under_test.SUPPLIER_NAME}_resp"
        ]
        ...
        for field in REQUIRED_FIELDS:
            assert getattr(
                dkpart, field
            ), f"Class {class_under_test} is missing required field {field}"
